...
const ViewTrends = () => {
  ...
  const calculateStatistics = (data) => {
    ...
    metrics.forEach(({ id }) => {
      const values = data.map(item => item[id]).filter(val => val !== undefined);
      ...
      calculatedStats[id] = {
        ...
        // Find the entries with min and max values
        minEntry: data.find(item => item[id] === Math.min(...values)),
        maxEntry: data.find(item => item[id] === Math.max(...values)),
  ...
  const generateMockData = (range) => {
    ...
    return Array.from({ length: dataPoints }, (_, i) => {
      ...
      return {
        date,
        temperature: 20 + Math.sin(i) * 5 + Math.random() * 2,
        humidity: 50 + Math.cos(i) * 20 + Math.random() * 5,
        gas_level: 10 + Math.sin(i/2) * 8 + Math.random() * 3,
        pressure: 1013 + Math.cos(i/3) * 10 + Math.random() * 2,
        altitude: 100 + Math.sin(i/4) * 50 + Math.random() * 10,
        uv_index: Math.floor(Math.abs(Math.sin(i/5) * 10 + Math.random() * 2))
      };
    }).reverse();
  };
  ...
  const prepareChartData = () => {
    ...
    const dataset = {
      ...
      backgroundColor: `${getColorForMetric(metric)}33`, // Add opacity
      tension: 0.1,
      fill: true,
    };
    ...
    return (
      <div className="analysis-details">
        <div className="analysis-row">
          ...
          <span className="analysis-value">{max.toFixed(1)}{metricInfo.unit}</span>
        </div>
        <div className="analysis-row">
          <span className="analysis-label">Lowest recorded:</span>
          <span className="analysis-value">{min.toFixed(1)}{metricInfo.unit}</span>
        </div>
        <div className="analysis-row">
          <span className="analysis-label">Average value:</span>
          <span className="analysis-value">{avg.toFixed(1)}{metricInfo.unit}</span>
        </div>
        <div className="trend-direction">
          The {metricInfo.name.toLowerCase()} {trendDirection} by {Math.abs(trend).toFixed(1)}{metricInfo.unit} 
          ({Math.abs(percentageChange).toFixed(1)}%) during this period.
        </div>
        <div className="metric-context">
          {getMetricContext(metric, avg)}
        </div>
      </div>
    );
  };
  ...
  const getMetricContext = (metric, avg) => {
    const contexts = {
      temperature: () => {
        if (avg > 30) return 'This is considered very hot. Take precautions against heat exposure.';
      ...
      humidity: () => {
        if (avg > 70) return 'High humidity may cause discomfort. Good for plants.';
        if (avg < 30) return 'Low humidity may cause dry skin. Consider a humidifier.';
        return 'Comfortable humidity level.';
      },
      gas_level: () => {
        if (avg > 50) return '⚠️ Warning: High gas levels detected. Ventilate area immediately.';
        if (avg > 30) return 'Elevated gas levels. Monitor closely.';
        return 'Normal gas levels detected.';
      },
      pressure: () => {
        if (avg > 1020) return 'High pressure typically indicates stable weather.';
        if (avg < 1000) return 'Low pressure often associated with stormy weather.';
        return 'Normal pressure range.';
      },
      altitude: () => {
        if (avg > 1500) return 'High altitude location. Be aware of potential altitude effects.';
        return 'Moderate altitude.';
      },
      uv_index: () => {
        if (avg > 8) return '⚠️ Extreme UV exposure. Avoid sun between 10am-4pm.';
        ...
        <>
          ...
          <div className="summary-cards">
            {metrics.filter(m => m.id !== metric).map(m => (
              <div key={m.id} className="summary-card" style={{ borderColor: getColorForMetric(m.id) }}>
                <h4>{m.name}</h4>
                {stats && stats[m.id] ? (
                  <>
                    <div className="summary-value">{stats[m.id].avg.toFixed(1)}{m.unit}</div>
                    <div className="summary-range">
                      {stats[m.id].min.toFixed(1)} - {stats[m.id].max.toFixed(1)}{m.unit}
                    </div>
                  </>
                ) : (
